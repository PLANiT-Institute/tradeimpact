...
| `country_emissions_us.csv` | `script/auto/country_emissions/extract_epa_inventory.py` | US `car_*`, `ldt_*` and `ldv_*` (cars + light-duty trucks) series: `*_co2` (ktCO2, Table 3-13 level) and `*_ghg_co2e` (ktCO2e, annual trend from Tables A-91/A-93). The benchmark uses `ldv_*` because EPA's passenger-car category is narrower than FHWA's light-duty stock and distance |
