...
| operation | `om_contractor` | `om_contract`, `operator_of_record` |
...
53 register rows for 12 companies from 12 pages on disk, by tier-2 scope: sponsor award 14, equity
direct 13, EPC consortium member 8, EPC lead 5, steam turbine supply 4, EPC scope not stated 3,
O&M contract 2, EPC civil works 2, boiler supply 2. The pages are KEPCO's own overseas history
list, BankTrack's Vung Ang II project page (KEPCO 40 %, Mitsubishi 40 %, Chugoku 20 %; Mitsubishi's
later 15 % sale to Shikoku is not yet on file, so no Shikoku row), the GEM wiki station page for
Jawa 9 and 10, Doosan's releases for Song Hau 1 and the Vinh Tan 4 extension, J-POWER's Gemeng
release, Sumitomo's Van Phong 1 release, Marubeni's Nghi Son 2 award and the project company's
shareholder page, AES Vietnam on Mong Duong 2, Toshiba's Matarbari order release and MHI's Jorf
Lasfar turbine release.

The hand register `raw/project_roles.csv` is still header-only. Next pages to read: J-POWER, JERA,
Marubeni, Sumitomo, Mitsui and the Korean gencos beyond KEPCO — their project lists are image or
script driven, so each row needs the project page or release that states the fact.
