...
Where TI sits against Scope 3 Category 11, avoided emissions and the other approaches to the
same question is set out in [`reference/landscape.md`](reference/landscape.md), over a register
of 83 works whose links are checked on every run.
...
reference/         the approaches TI sits beside: references.csv (register, with where TI
                   departs from each), landscape.md (the comparison), link_check.csv and
                   raw/index.csv (generated). raw/ itself is a local reading library, out of
                   git; script/reference/fetch_references.py rebuilds it from the register
...
have but cannot yet assess. The denominator covers the brands the company reports together, and
...
America full years 2024 and 2025; Kia IR Jan–Jul 2026, a part year); the US S1 benchmark is
...
default otherwise, read by hand from each destination's own inventory report or legal instrument
with the table line quoted and the arithmetic re-done by the extractor; the capacity factor is the destination's own utilisation of that fuel from
