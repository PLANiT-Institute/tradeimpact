...
table.data td.neg { color: var(--muted); }
...
const MAP_METRICS = [
  ...
  {id: 'ti_tco2e', label: 'lifetime TI (tCO2e), + added / - avoided', kind: 'div',
   unit: 'tCO2e',
   sql: (f) => 'SELECT destination, SUM(ti_tco2e) FROM ti_country WHERE scenario = ' +
     lit(f.scenarioVal) + f.company + f.year + ' GROUP BY 1'},
  {id: 'ti_per_vehicle', label: 'TI per vehicle (kgCO2e), + added / - avoided',
   kind: 'div', unit: 'kgCO2e per vehicle',
...
function renderMapShell() {
  ...
  let h = '<section class="card map"><h2>Map</h2>' +
    ...
    sel('map-scenario', 'scenario',
      ['S1', 'S2'].map((sc) => opt(sc, sc, sc === m.scenario)).join('')) +
...
function mapColor(metric, nums) {
  ...
  if (metric.kind === 'div') {
    ...
    // TI is emissions minus benchmark, so the positive end is emissions added: red. The
    // domain runs high-to-low because interpolateRdBu puts red at its low end.
...
function renderMapLegend(metric, color, nums) {
  ...
  el.innerHTML = stops.map((v) => swatch(color(v), fmtNum(v))).join('') +
    ...
    (metric.kind === 'div'
      ? ' (red = emissions added, blue = emissions avoided)' : '') + '</span>' +
